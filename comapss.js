...
ctx.clip();
ctx.stroke();//innercircle
mouseSector = Math.ceil(((Math.atan2(mY-wY/2,mX-wX/2))/(circleOffset))+distributionDivision)
pageSelect();
eventclick = false;
ctx.beginPath();
ctx.clearRect(0, 0, wX, wY);
...
ctx.restore();
  ...
  document.getElementById('ang').innerText = (Math.atan2((mX-wX/2), (mY-wY/2)));
